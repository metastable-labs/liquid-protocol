...
contract AerodromeConnectorTest is Test {
    ...
    address public constant USDC_WETH_GAUGE =
    ...
    function testAddLiquidity() public {
        ...
        (uint256 amountA, uint256 amountB, uint256 liquidity) = abi.decode(
            result,
            (uint256, uint256, uint256)
        );

        console.log(
            "Liquidity added: %s USDC, %s WETH, %s LP",
            amountA,
            amountB,
            liquidity
        );
    ...
    function testRemoveLiquidity() public {
        ...
        address pair = IPoolFactory(AERODROME_FACTORY).getPool(
            USDC,
            WETH,
            false
        );
        ...
        try connector.execute(data) returns (bytes memory result) {
            ...
            console.log(
                "Liquidity removed: %s USDC, %s WETH",
                amountA,
                amountB
            );
    ...
    function testSwapExactTokensForTokens() public {
        ...
        uint256[] memory expectedAmounts = IRouter(AERODROME_ROUTER)
        ...
        assertGt(
            amounts[amounts.length - 1],
            amountOutMin,
            "Output amount should be greater than minimum"
        );
        assertGe(
            amounts[amounts.length - 1],
            minReturnAmount,
            "Output amount should be greater than or equal to minimum"
        );
    ...
    //     IERC20(USDC_WETH_POOL).approve(USDC_WETH_GAUGE, depositAmount);
    ...
    // function testWithdrawFromGauge() public {
    //     // First, deposit LP tokens into the gauge
    //     testDepositToGauge();

    //     uint256 initialGaugeBalance = IGauge(USDC_WETH_GAUGE).balanceOf(ALICE);
    //     require(initialGaugeBalance > 0, "No tokens staked in gauge");

    //     uint256 initialLPBalance = IERC20(USDC_WETH_POOL).balanceOf(ALICE);

    //     vm.startPrank(ALICE);

    //     // Prepare the withdrawal data
    //     bytes memory data = abi.encodeWithSelector(IGauge.withdraw.selector, USDC_WETH_GAUGE, initialGaugeBalance);

    //     // Execute the withdrawal through the connector
    //     bytes memory result = connector.execute(data);
    //     uint256 withdrawnAmount = abi.decode(result, (uint256));

    //     // Verify the withdrawal
    //     uint256 finalGaugeBalance = IGauge(USDC_WETH_GAUGE).balanceOf(ALICE);
    //     assertLe(
    //         finalGaugeBalance,
    //         initialGaugeBalance - withdrawnAmount,
    //         "Gauge balance should decrease by at least the withdrawn amount"
    //     );

    //     uint256 finalLPBalance = IERC20(USDC_WETH_POOL).balanceOf(ALICE);
    //     assertEq(
    //         finalLPBalance, initialLPBalance + withdrawnAmount, "LP token balance should increase by withdrawal amount"
    //     );

    //     vm.stopPrank();
    // }

    // function testPartialWithdrawFromGauge() public {
    //     // First, deposit LP tokens into the gauge
    //     testDepositToGauge();
    ...
    //     uint256 initialLPBalance = IERC20(USDC_WETH_POOL).balanceOf(ALICE);
    //     uint256 withdrawAmount = initialGaugeBalance / 2; // Withdraw half of the staked amount

    //     vm.startPrank(ALICE);

    //     // Prepare the withdrawal data
    //     bytes memory data = abi.encodeWithSelector(IGauge.withdraw.selector, USDC_WETH_GAUGE, withdrawAmount);
    ...
    // function testOverWithdrawFromGauge() public {
    //     // First, deposit LP tokens into the gauge
    //     testDepositToGauge();
    ...
    //     uint256 withdrawAmount = initialGaugeBalance + 1; // Try to withdraw more than staked
    ...
    // function testGaugeBehavior() public {
    //     // First, deposit LP tokens into the gauge
    //     testDepositToGauge();
    ...
    //     // Try to withdraw 1 token
    //     try IGauge(USDC_WETH_GAUGE).withdraw(1) {
    //         console.log("Withdrawing 1 token succeeded");
    //     } catch Error(string memory reason) {
    //         console.log("Withdrawing 1 token failed:", reason);
    //     } catch {
    //         console.log("Withdrawing 1 token failed with no reason");
    //     }

    //     // Try to withdraw all tokens
    //     try IGauge(USDC_WETH_GAUGE).withdraw(initialGaugeBalance) {
    //         console.log("Withdrawing all tokens succeeded");
    //     } catch Error(string memory reason) {
    //         console.log("Withdrawing all tokens failed:", reason);
    //     } catch {
    //         console.log("Withdrawing all tokens failed with no reason");
    //     }
    ...
    function testInvalidSelector() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(AerodromeConnector.InvalidSelector.selector)
        );
        connector.execute(data);
    }
}
