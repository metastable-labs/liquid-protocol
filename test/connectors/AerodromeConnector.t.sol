...
contract AerodromeConnectorTest is Test {
    ...
    function testClaimPoolFees() public {
        ...
        uint256 initialBalance0 = IERC20(USDC).balanceOf(ALICE);
        uint256 initialBalance1 = IERC20(WETH).balanceOf(ALICE);

        // Warp time to allow fees to accumulate
        vm.warp(block.timestamp + 7 days);

        vm.startPrank(ALICE);

        // Prepare the claim fees data
        bytes memory data = abi.encodeWithSelector(IPool.claimFees.selector, USDC_WETH_POOL);

        uint256 claimable0 = IPool(USDC_WETH_POOL).claimable0(ALICE);
        uint256 claimable1 = IPool(USDC_WETH_POOL).claimable0(ALICE);
        console.log("Claimable fees before claim - USDC: %s, WETH: %s", claimable0, claimable1);

        // Execute the fee claim through the connector
        bytes memory result = connector.execute(data);
        (uint256 claimed0, uint256 claimed1) = abi.decode(result, (uint256, uint256));

        console.log("Claimed fees - USDC: %s, WETH: %s", claimed0, claimed1);

        // Verify the fee claim
        uint256 finalBalance0 = IERC20(USDC).balanceOf(ALICE);
        uint256 finalBalance1 = IERC20(WETH).balanceOf(ALICE);

        assertEq(finalBalance0, initialBalance0 + claimed0, "USDC balance should increase by claimed amount");
        assertEq(finalBalance1, initialBalance1 + claimed1, "WETH balance should increase by claimed amount");

        // Check that at least some fees were claimed
        assertTrue(claimed0 > 0 || claimed1 > 0, "No fees were claimed");
    ...
    function simulateSwaps() internal {
        ...
        for (uint256 i = 0; i < 20; i++) {
            ...
            IERC20(USDC).approve(address(router), swapAmount);
            router.swapExactTokensForTokens(swapAmount, 0, getRoute(USDC, WETH), ALICE, block.timestamp);

            // Swap back
            uint256 ethBalance = IERC20(WETH).balanceOf(ALICE);
            IERC20(WETH).approve(address(router), ethBalance);
            router.swapExactTokensForTokens(ethBalance, 0, getRoute(WETH, USDC), ALICE, block.timestamp);
        }
    ...
    function getRoute(address tokenIn, address tokenOut) internal view returns (IRouter.Route[] memory) {
        IRouter.Route[] memory routes = new IRouter.Route[](1);
        routes[0] = IRouter.Route({from: tokenIn, to: tokenOut, stable: false, factory: AERODROME_FACTORY});
        return routes;
    }
